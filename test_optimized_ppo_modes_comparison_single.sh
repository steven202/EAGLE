...
if [ ${#DIRECTORIES_TO_CREATE[@]} -gt 0 ]; then
    ...
    for dir in "${DIRECTORIES_TO_CREATE[@]}"; do
        echo "ofl [256, 256]" | tee -a log/$DATE/$dir/execution_config.txt
        echo "ofl [256, 256]" | tee -a log/$DATE/$dir/summary.txt
        echo "ofl [256, 256]" | tee -a log/$DATE/$dir/comparison.txt
